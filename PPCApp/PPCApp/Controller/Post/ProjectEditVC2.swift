...
import Foundation
import UIKit
class ProjectEditVC2: BaseVC,UITextFieldDelegate {
    ...
    func handleNextButton(_ sender: UIButton){
        if checkInputProjectVC2(){
            let proEditVC3 = ProjectEditVC3()
            proEditVC3.post = self.post
            push(viewController: proEditVC3)
        }else{
